...
{
	CheckEthHistTable();  //履歴は、毎秒チェックする。
	CheckAlarm();
	switch( nTimerCount ){
	case 1:
		CheckAddrMapTime();
...
struct nlHostTable_entry {
    ...
    long protocolDirLocalIndex;
    char nlHostAddress[16]; // 一応、IPv6対応
...
struct alHostTable_entry {
    ...
    long protocolDirLocalIndexNl; //こちらは、ネットワーク層の識別、主にＩＰ
    char nlHostAddress[16];
    size_t nlHostAddress_len;
    long protocolDirLocalIndexAl; //こちらは、ポートまで含めたＡＰ層の識別
...
void UpdateHost(int nDir,int nSMode,char *szMac,int nLen)
{
	...
	if( nDir == 1 ) {  //送受信が逆でした。YMI 2007.7.5
		p->hostOutPkts++;
...
{
	//統計グループ（パケット数、バイト数、あて先別、パケットサイズ別）
	//履歴グループ
	int nSMode =0;
...
void SetDataSource(char *szDev)
{
	...
	while( fgets(szLine,sizeof(szLine)-1,fp) ) {
		if( strchr(szLine,':') == NULL ) continue;  // eth1:のような形式のみカウント
...
// 以下は、TimeMarkのために定期的に実行する処理です。概ね１０秒毎に呼ばれる？
